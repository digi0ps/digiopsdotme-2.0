...
const ArticleBoxOnClick = (id, push, e) => {
  ...
  for (let i = 0; i < all_text.length; i++) all_text[i].style.color = "white";

  const t = TweenLite.fromTo(
    box,
    0.2,
    { position: "absolute", top: top, left: left, color: "white" },
    { top: "-10px", bottom: "100px", left: 0, width: width, height: height },
  );
...
const Article = props => {
  ...
  return (
    ...
    >
      <div>
        <h3>{title}</h3>
        <p className="article-box-body" dangerouslySetInnerHTML={html} />
...
class Blog extends React.Component {
  ...
  fetchArticles() {
    ...
    const endpoint = `${host}/blog/api/articles/`;
  ...
  renderArticles() {
    if (!this.state.loaded) return <div className="load">Loading...</div>;
  ...
  renderAnimatedArticles() {
    ...
    else return this.renderArticles();
  }
    ...
    return (
      <div className="blog-con">
        <Helmet>
          ...
          <meta
            name="description"
            content="A tiny exhibition of my musings and ramblings."
          />
        </Helmet>
        <HomeLink />
        <div className="blog center">
          ...
          <div className="articles">{this.renderAnimatedArticles()}</div>
        </div>
      </div>
    );
  }
